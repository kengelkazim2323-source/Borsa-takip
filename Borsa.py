...
st.markdown(f"""
    ...
    /* MOBİL TABLO ÖZELLEŞTİRMESİ */
    .mobile-scroll {{
        overflow-x: auto;
        white-space: nowrap;
        border: 1px solid {t_sec['accent']}33;
        border-radius: 10px;
        background: {t_sec['box']}44;
    }}
    .kral-table {{
        width: 100%;
    ...
    .kral-table th {{
        ...
        color: {t_sec['accent']};
        font-size: 13px;
        border-bottom: 1px solid {t_sec['accent']}55;
    }}
    .kral-table td {{
        padding: 12px;
...
with st.expander("➕ PORTFÖYE VARLIK EKLE", expanded=False):
...
if st.session_state.portfoy:
    tab_tr, tab_us, tab_div = st.tabs(["🇹🇷 TÜRK BORSASI", "🇺🇸 AMERİKAN BORSASI", "💰 TEMETTÜ"])
    ...
    def portfoy_goster(piyasa_turu, tab_container, data_list):
        with tab_container:
            ...
            # MOBİL UYUMLU KAYDIRILABİLİR TABLO
            html_table = f"""
            ...
            for _, r in df.iterrows():
                ...
                html_table += f"""
                    <tr>
                        <td><b>{r['Hisse']}</b><br><small>{r['Sinyal']}</small></td>
                        <td>{r['Adet']}</td>
            ...
            # İşlem Butonları (Mobilde yan yana sütunlar bozulduğu için liste olarak alta ekledim)
            with st.expander("⚙️ VARLIKLARI YÖNET (SİL)"):
                for _, r in df.iterrows():
                    if st.button(f"❌ {r['Hisse']} Varlığını Sil", key=f"del_{r['id']}"):
    ...
    with tab_div:
        ...
        if not df_div.empty:
            st.subheader("💰 Beklenen Nakit Akışı")
            div_html = '<div class="mobile-scroll"><table class="kral-table"><thead><tr><th>VARLIK</th><th>ADET</th><th>NET GELİR</th></tr></thead><tbody>'
...
st.caption(f"🕒 Son Güncelleme: {tr_saati}")
