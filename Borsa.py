...
# ==========================================
# 0. SAYI FORMATLAMA VE GELİŞMİŞ TEKNİK ANALİZ (GÜNCELLENDİ)
# ==========================================
def tr_format(val):
    try:
...
def get_signal(data):
    """RSI, MACD ve Hareketli Ortalama tabanlı gelişmiş sinyal motoru."""
    try:
        ...
        # Doğruluk oranı yüksek kombo karar
        buy_score = 0
...
# ==========================================
# 2. GÖRSEL AYARLAR VE SAAT (DOKUNULMADI)
# ==========================================
...
# ==========================================
# 4. TAM BİST LİSTESİ (DOKUNULMADI)
# ==========================================
...
# ==========================================
# 5. VERİ ANALİZİ VE TEMETTÜ HESABI (GÜNCELLENDİ)
# ==========================================
if st.session_state.portfoy:
    ...
    for item in st.session_state.portfoy:
        ...
        try:
            ...
            hist = tk.history(period="60d") # Analiz doğruluğu için 60 gün
            ...
            if not hist.empty and len(hist) >= 2:
                ...
                # --- TEMETTÜ GARANTİLEME SİSTEMİ ---
                # Doğrudan son 1 yıllık ödeme geçmişine bakıyoruz
                divs = tk.dividends
                if not divs.empty:
                    one_year_ago = datetime.now() - timedelta(days=365)
                    divs.index = divs.index.tz_localize(None) # Tarih hatasını engeller
                    h_temettu = divs[divs.index > one_year_ago].sum()
    ...
    if not df.empty:
        ...
        with tab1:
            ...
            for c in format_cols: 
                if c in df_disp.columns: df_disp[c] = df_disp[c].apply(tr_format)
        ...
        with tab3:
            ...
            st.success(f"### Yıllık Tahmini Net Temettü: {tr_format(toplam_t)} ₺")
            div_view = df[df['Lot Başı Temettü'] > 0][["Varlık", "Adet", "Lot Başı Temettü", "Net Temettü (₺)"]]
            if not div_view.empty:
                st.table(div_view.style.format({"Lot Başı Temettü": "{:,.2f} ₺", "Net Temettü (₺)": "{:,.2f} ₺"}))
            else:
                st.warning("Seçili hisseler için son 1 yılda ödenmiş temettü verisi bulunamadı.")
