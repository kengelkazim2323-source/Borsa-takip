...
# ==========================================
# 0. SAYI FORMATLAMA (TR STANDARDI: 1.250,50)
# ==========================================
def tr_format(val):
    try:
...
# ==========================================
piyasa_izleme = {
    "BIST 100": "XU100.IS", "GRAM ALTIN": "GAU-TRY", 
    "ONS ALTIN": "GC=F", "ONS GÜMÜŞ": "SI=F", "USD/TRY": "USDTRY=X"
}
...
# ==========================================
# 4. HİSSE LİSTESİ VE GÜZELLEŞTİRİLMİŞ EKLEME ALANI
# ==========================================
...
# Widget Karmaşasını Önlemek İçin Container Kullanımı
with st.container():
    col_v, col_a, col_m = st.columns([2, 1, 1])
    with col_v: s_varlik = st.selectbox("Hisse/Varlık", BIST_FULL)
...
if st.session_state.portfoy:
    ...
    for item in st.session_state.portfoy:
        try:
            ...
            # Temettü Hesabı
            info = tk.info
            d_rate = info.get('dividendRate', 0) or 0
            if not d_rate and info.get('dividendYield'):
                d_rate = curr * info.get('dividendYield')
            
            p_data.append({
                "Varlık": item['Hisse'], "Adet": item['Adet'], "Maliyet": item['Maliyet'],
                "Güncel": curr, "Değer": val, "K/Z": kz, "Yıllık Temettü": d_rate * float(item['Adet'])
            })
        except: continue
    ...
    with tab1:
        ...
        st.dataframe(df_disp.drop(columns=["Yıllık Temettü"]), use_container_width=True, hide_index=True)

    with tab2:
        fig = px.pie(df, values='Değer', names='Varlık', hole=0.5, 
                     color_discrete_sequence=px.colors.qualitative.Safe)
        fig.update_layout(showlegend=True, margin=dict(t=20, b=20, l=0, r=0))
        st.plotly_chart(fig, use_container_width=True)

    with tab3:
        top_div = df['Yıllık Temettü'].sum()
        st.info(f"### Yıllık Tahmini Temettü: {tr_format(top_div)} ₺")
        st.write(f"**Aylık Ortalama:** {tr_format(top_div/12)} ₺")
    ...
    st.warning("Portföyünüz henüz boş. Yukarıdan varlık ekleyerek takibe başlayabilirsiniz.")
