...
# ==========================================
# 1. VERİ YÖNETİMİ (DOSYA TABANLI - SIFIRLANMAZ)
# ==========================================
PORTFOY_DOSYASI = "portfoy_verileri.json"

def load_data():
    """Dosyadan verileri oku, dosya yoksa boş liste döndür."""
...
def save_data(data):
    """Verileri kalıcı olarak dosyaya yaz."""
...
# ==========================================
# 2. KRİTİK AYARLAR VE GİZLEME (CSS)
# ==========================================
...
st.markdown("""
    ...
    /* ÜST PANEL */
    .ticker-wrapper { width: 100%; overflow-x: auto; background: #161b22; border-bottom: 1px solid #30363d; position: fixed; top: 0; left: 0; right: 0; z-index: 9999; }
...
for isim, sembol in piyasa_izleme.items():
    try:
        tk = yf.Ticker(sembol)
        fiyat = tk.fast_info['lastPrice']
        degisim = ((fiyat - tk.fast_info['regularMarketPreviousClose']) / tk.fast_info['regularMarketPreviousClose']) * 100
...
with col4: 
    if st.button("🚀 EKLE", use_container_width=True):
        ...
        save_data(st.session_state.portfoy) # Dosyaya kaydet
        st.rerun()

# ==========================================
# 5. SEKME MANTIĞI VE ANALİZ
# ==========================================
...
if st.session_state.portfoy:
    ...
    for item in st.session_state.portfoy:
        try:
            ...
            # Net Temettü (%10 Stopajlı)
            info = tk.info
            brut_hisse_basi = info.get('dividendRate', 0) or 0
            verim_pct = info.get('dividendYield', 0) or 0
            net_hisse_basi = brut_hisse_basi * 0.90
            toplam_net_nakit = item['Adet'] * net_hisse_basi
            ...
            p_data.append({
                ...
                "Verim": f"%{verim_pct*100:.2f}",
                "Hisse Başı Net": net_hisse_basi,
                "Toplam Net Nakit": toplam_net_nakit
            })
        except: continue
    ...
    with tab_p:
        ...
        if st.button("🗑️ Portföyü Sıfırla"):
            ...
            save_data([]) # Dosyayı temizle
            st.rerun()

    with tab_s:
        for _, row in df.iterrows():
            st.markdown(f'<div class="signal-box"><b>{row["Hisse"]}</b> → {row["Sinyal"]} <br><small>RSI: {row["RSI"]:.2f} | Fiyat: {row["Güncel"]:.2f}</small></div>', unsafe_allow_html=True)

    with tab_t:
        total_net = df['Toplam Net Nakit'].sum()
        c1, c2 = st.columns(2)
        c1.metric("Yıllık Toplam Net Nakit", f"{total_net:,.2f} ₺")
        c2.metric("Portföy Verimi", f"%{(total_net/df['Değer'].sum()*100):.2f}" if df['Değer'].sum() > 0 else "%0")
        
        st.divider()
        st.dataframe(df[['Hisse', 'Adet', 'Verim', 'Hisse Başı Net', 'Toplam Net Nakit']].style.format({
            "Hisse Başı Net": "{:.2f} ₺", "Toplam Net Nakit": "{:,.2f} ₺"
