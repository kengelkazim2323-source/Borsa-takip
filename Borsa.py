...
def fetch_tefas_price(symbol):
    try:
        # Yatırım fonları için anlık fiyat çekme (TEFAS)
...
for i, item in enumerate(st.session_state.portfoy):
    ...
    if piyasa_durumu == "Yatırım Fonu":
        # TEFAS Üzerinden Anlık Yatırım Fonu Çekimi
        fon_fiyati = fetch_tefas_price(item['Hisse'])
        if fon_fiyati:
            c = fon_fiyati
            pc = fon_fiyati # Önceki kapanış fonlar için karmaşık, fiyat eşit varsayılıyor
            sinyal = "VERİ YOK"
            temettu = 0.0
        else:
        ...
        # Normal Borsa Hissesi Çekimi (YFinance)
...
with tab_div:
    ...
    if not df_div.empty:
        c1, c2 = st.columns(2)
        c1.metric("BIST TEMETTÜ", f"{tr_format(df_div[df_div['Piyasa'] == 'Türk Borsası']['NetTemettu'].sum())} ₺")
        c2.metric("FON TEMETTÜ", f"{tr_format(df_div[df_div['Piyasa'] == 'Yatırım Fonu']['NetTemettu'].sum())} ₺")
        for _, r in df_div[df_div['NetTemettu'] > 0].sort_values(by="NetTemettu", ascending=False).iterrows():
    ...
    if st.session_state.ipo_liste:
        for idx, ipo in enumerate(st.session_state.ipo_liste):
            with st.container():
                ...
                toplam_maliyet = ipo['Adet'] * ipo['Fiyat']
                col1.markdown(f"### {ipo['Isim']}")
                col2.write(f"Maliyet: **{tr_format(toplam_maliyet)} ₺** ({ipo['Adet']} Adet)")
...
if full_data:
    ...
    if not df_chart.empty:
        ...
        if not df_fon.empty:
            ...
            fig_fon.update_layout(showlegend=True, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color=t_sec['text']), margin=dict(t=50, b=30, l=30, r=30), legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.25), title_x=0.5)
