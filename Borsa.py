...
def load_data():
    """Dosyadan verileri yükle. Dosya yoksa veya bozuksa boş liste döner."""
...
# Başlangıçta veriyi çek ve session_state'e sabitle
if 'portfoy' not in st.session_state:
    st.session_state.portfoy = load_data()

# Sayfayı 15 saniyede bir yenile
try:
    from streamlit_autorefresh import st_autorefresh
...
# Tema Seçici (Kenar Çubuğu)
with st.sidebar:
    st.title("⚙️ AYARLAR")
    tema_secimi = st.radio("Terminal Teması", ["Matrix Green", "Cyber Blue", "Volcanic Red"])
...
glow_color = themes[tema_secimi]["glow"]
...
st.markdown(f"""
    ...
    .up {{ color: {main_color}; text-shadow: 0 0 10px {glow_color}; }}
    .down {{ color: #ff3131; }}
...
for isim, sembol in piyasa_izleme.items():
    try:
        ...
        renk_class = "up" if degisim >= 0 else "down"
        ticker_content += f'<div class="ticker-card"><span class="{renk_class}" style="font-weight:800">{degisim:+.2f}%</span><span style="font-size:10px; color:#8b949e">{isim}</span><span style="font-weight:600">{last:,.2f}</span></div>'
    except: continue
...
# ==========================================
# 5. GİRİŞ PANELİ (TAM SAYI MODU)
# ==========================================
st.markdown(f"<h1 style='text-align: center; color:{main_color};'>🏛️ BIST QUANTUM TERMINAL</h1>", unsafe_allow_html=True)

with st.container():
    c1, c2, c3, c4 = st.columns([3, 2, 2, 1])
    with c1: s_hisse = st.selectbox("Varlık Ara", BIST_FULL, label_visibility="collapsed")
    # Step=1 ve format="%d" ile ondalıkları kaldırdık
    with c2: s_adet = st.number_input("Adet", min_value=0, step=1, value=0, label_visibility="collapsed", format="%d")
    with c3: s_maliyet = st.number_input("Maliyet", min_value=0, step=1, value=0, label_visibility="collapsed", format="%d")
    with c4: 
        if st.button("🚀 EKLE", use_container_width=True):
            if s_adet > 0:
                st.session_state.portfoy.append({"Hisse": s_hisse, "Adet": int(s_adet), "Maliyet": int(s_maliyet)})
...
if st.session_state.portfoy:
    ...
    with tab_g:
        if not df.empty:
            st.bar_chart(df.set_index('Hisse')['K/Z'], color=main_color)

    with tab_s:
        for _, row in df.iterrows():
            st.markdown(f'<div class="signal-box" style="border-left-color: {main_color}"><b>{row["Hisse"]}</b>: {row["Sinyal"]} (RSI: {row["RSI"]:.2f})</div>', unsafe_allow_html=True)
