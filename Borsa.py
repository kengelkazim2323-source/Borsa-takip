...
with st.sidebar:
    ...
    tema = st.selectbox("Tema Seçimi", ["Premium Koyu", "Galaksi (VIP)", "Altın Vuruş", "Zümrüt Yeşili", "Siber Punk", "Retro Kehribar", "Matrix"])
...
st.markdown(f"""
    ...
    table.kral-table {{
        ...
        min-width: 600px; /* Mobilde kaydırma sağlamak için */
        background: {t_sec['box']};
    }}
    table.kral-table th {{
    ...
    .signal-badge {{ font-size: 10px; font-weight: bold; opacity: 0.8; }}
    </style>
...
# 3. CANLI PİYASA
# ==========================================
st.markdown(f"<h2 style='text-align:center; color:{t_sec['accent']};'>🚀 KRAL TERMİNAL</h2>", unsafe_allow_html=True)
piyasa_izleme = { "BIST 100": "XU100.IS", "ALTIN": "GC=F", "GÜMÜŞ": "SI=F", "GRAM": "GAU-TRY", "USD/TRY": "USDTRY=X", "BTC": "BTC-USD"}
ticker_content = '<div class="ticker-wrapper" style="overflow-x:auto;"><div style="display:flex; gap:30px; padding:10px;">'
for isim, sembol in piyasa_izleme.items():
    ...
    if d:
        ...
        ticker_content += f'<div style="min-width:100px; text-align:center;"><div style="font-size:10px;">{isim}</div><div style="font-weight:bold;">{tr_format(last)}</div><div style="color:{"#00e676" if deg>=0 else "#ff1744"}; font-size:12px;">{deg:+.2f}%</div></div>'
...
# ==========================================
# 5. LİSTELEME (MOBİL UYUMLU TABLO)
# ==========================================
if st.session_state.portfoy:
    tab_tr, tab_us, tab_div = st.tabs(["🇹🇷 TÜRK", "🇺🇸 ABD", "💰 TEMETTÜ"])
    ...
    def portfoy_goster(piyasa_turu, tab_container, data_list):
        with tab_container:
            ...
            m1.metric("DEĞER", f"{tr_format(df['Değer'].sum())} {birim}")
            m2.metric("K/Z", f"{tr_format(df['K/Z'].sum())} {birim}")
            m3.metric("GÜN", f"{tr_format(df['DailyDiff'].sum())}", delta=f"{df['DailyDiff'].sum():,.2f}")

            # MOBİL UYUMLU HTML TABLO
            html_table = f"""
            ...
            for _, r in df.iterrows():
                ...
                html_table += f"""
                    <tr>
                        <td><b>{r['Hisse']}</b><br><span class="signal-badge">{r['Sinyal']}</span></td>
                        <td>{r['Adet']}</td>
                        <td>{tr_format(r['Maliyet'])}</td>
                        <td>{tr_format(r['Güncel'])}</td>
                        <td style="color:{kz_color}; font-weight:bold;">{tr_format(r['K/Z'])}</td>
            ...
            # Silme Butonları (Tablo altında mobil için daha güvenli)
            with st.expander("⚙️ İŞLEMLER (VARLIK SİL)"):
                for _, r in df.iterrows():
                    if st.button(f"❌ {r['Hisse']} SİL", key=f"del_{r['id']}"):
    ...
    with tab_div:
        ...
        if not df_div.empty:
            st.subheader("💰 Yıllık Nakit Akışı")
            c1, c2 = st.columns(2)
            c1.metric("BIST", f"{tr_format(df_div[df_div['Piyasa'] == 'Türk Borsası']['NetTemettu'].sum())} ₺")
            c2.metric("ABD", f"{tr_format(df_div[df_div['Piyasa'] == 'Amerikan Borsası']['NetTemettu'].sum())} $")
            
            div_html = '<div class="mobile-table-container"><table class="kral-table"><thead><tr><th>VARLIK</th><th>ADET</th><th>NET GELİR</th></tr></thead><tbody>'
            for _, r in df_div.iterrows():
                if r['Temettu'] > 0:
                    birim = "₺" if r['Piyasa'] == "Türk Borsası" else "$"
                    div_html += f"<tr><td>{r['Hisse']}</td><td>{r['Adet']}</td><td><b>{tr_format(r['NetTemettu'])} {birim}</b></td></tr>"
            st.markdown(div_html + "</tbody></table></div>", unsafe_allow_html=True)

    if st.button("🗑️ TÜMÜNÜ TEMİZLE"):
        st.session_state.portfoy = []; save_data([]); st.rerun()
else:
    st.info("Portföy boş kral.")
