...
# ==========================================
# 1. GÖRSEL AYARLAR VE FONT (JETBRAINS MONO)
# ==========================================
st.set_page_config(page_title="BORSA TAKİP", layout="wide")
st_autorefresh(interval=500, key="refresh") # 0.5 sn refresh

st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;700&display=swap');
    html, body, [class*="st-"] { font-family: 'JetBrains Mono', monospace; }
    .ticker-wrapper { width: 100%; overflow: hidden; background: #1e1e1e; color: #00ff00; padding: 10px 0; margin-bottom: 20px; border-radius: 5px; }
    .ticker-content { display: flex; animation: ticker 30s linear infinite; white-space: nowrap; gap: 50px; }
    @keyframes ticker { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } }
    .stMetric { background: #f8f9fa; padding: 10px; border-radius: 10px; border-left: 5px solid #1a73e8; }
    </style>
    """, unsafe_allow_html=True)

# ==========================================
# 2. ÜST PANEL: CANLI PİYASA AKIŞI (TICKER)
# ==========================================
piyasa_list = {"BIST 100": "XU100.IS", "GRAM ALTIN": "GAU-TRY", "USD/TRY": "USDTRY=X", "ONS ALTIN": "GC=F", "GÜMÜŞ": "SI=F"}
ticker_html = '<div class="ticker-wrapper"><div class="ticker-content">'
for isim, sembol in piyasa_list.items():
    d = get_data(sembol)
    if d:
        p = d['last_price']
        change = ((p - d['hist']['Close'].iloc[-2]) / d['hist']['Close'].iloc[-2]) * 100
        ticker_html += f'<span>{isim}: <b>{tr_format(p)}</b> <small>({change:+.2f}%)</small></span>'
ticker_html += '</div></div>'
st.markdown(ticker_html, unsafe_allow_html=True)

tr_saati = datetime.now(pytz.timezone('Europe/Istanbul')).strftime('%H:%M:%S')
st.markdown(f"**🕒 Son Güncelleme:** {tr_saati}")
...
if 'portfoy' not in st.session_state:
    if os.path.exists(PORTFOY_FILE):
        with open(PORTFOY_FILE, "r") as f: st.session_state.portfoy = json.load(f)
...
with st.container():
    ...
    if st.button("🚀 PORTFÖYE EKLE", use_container_width=True):
        if h_adet > 0:
            ...
            with open(PORTFOY_FILE, "w") as f: json.dump(st.session_state.portfoy, f)
            st.rerun()

# ==========================================
# 5. TABLOLAR VE DAİRESEL GRAFİK
# ==========================================
if st.session_state.portfoy:
    ...
    tab1, tab2, tab3 = st.tabs(["📊 PORTFÖY", "📈 DAĞILIM", "💰 TEMETTÜ"])
    ...
    with tab2:
        ...
        fig = px.pie(df, values='Değer', names='Varlık', hole=0.4, color_discrete_sequence=px.colors.qualitative.Pastel)
...
if st.button("🗑️ TÜMÜNÜ TEMİZLE"):
    ...
    if os.path.exists(PORTFOY_FILE): os.remove(PORTFOY_FILE)
    st.rerun()
