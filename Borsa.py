...
# ==========================================
# 4. VARLIK EKLEME PANELİ (DİNAMİK SATIR TETİKLEYİCİ)
# ==========================================
...
if st.session_state.portfoy:
    ...
    for item in st.session_state.portfoy:
        try:
            ...
            yillik_net = (d_rate * item['Adet']) * 0.85
    ...
    with tab_p:
        ...
        # Her ekleme sonrası burası otomatik olarak yeni satırlarla dolar
        df_edit = df[['Varlık', 'Adet', 'Maliyet']].copy()
        df_edit['Güncel Fiyat'] = df['Güncel'].apply(tr_format)
        df_edit['Kâr/Zarar'] = df['K/Z'].apply(tr_format)
        ...
        # Tablodan manuel değişiklik yapılırsa kaydet
        if not edited[['Adet', 'Maliyet']].equals(df_edit[['Adet', 'Maliyet']]):
            new_port = []
            for i, row in edited.iterrows():
                new_port.append({"Hisse": row['Varlık'], "Adet": int(row['Adet']), "Maliyet": float(row['Maliyet'])})
            st.session_state.portfoy = new_port
            save_data(new_port)
            st.rerun()

    with tab_g:
        fig = px.pie(df, values='Değer', names='Varlık', title="Varlık Dağılımı", hole=0.4)
    ...
    with tab_s:
        for _, r in df.iterrows():
            ...
            st.write(f"**{r['Varlık']}**: RSI {r['RSI']:.2f} -> {durum}")
else:
