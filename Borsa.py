...
# ==========================================
def tr_format(val):
    """Sayıları Türkçe formata (1.234,56) çevirir"""
...
# ==========================================
# 4. VARLIK LİSTESİ (BÜTÜN BIST HİSSELERİ)
# ==========================================
...
VARLIK_LISTESI = ["GAU-TRY", "GAG-TRY", "USDTRY=X", "EURTRY=X", "BTC-USD", "ETH-USD"] + BIST_FULL
...
# 6. TABLAR
# ==========================================
tab_p, tab_g, tab_t, tab_s = st.tabs(["📊 PORTFÖY", "📈 ANALİZ", "💰 TEMETTÜ", "🤖 SİNYAL"])
...
if st.session_state.portfoy:
    ...
    with tab_p:
        ...
        st.markdown("<small><i>💡 Tablo içindeki Adet ve Maliyet değerlerine çift tıklayarak düzenleyebilirsiniz.</i></small>", unsafe_allow_html=True)
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Maliyet": st.column_config.NumberColumn("Maliyet", min_value=0, step=1, format="%d"),
                "Güncel (₺)": st.column_config.TextColumn("Güncel Fiyat", disabled=True),
                "Değer (₺)": st.column_config.TextColumn("Toplam Değer", disabled=True),
                "K/Z (₺)": st.column_config.TextColumn("Kar/Zarar", disabled=True),
            },
            hide_index=True,
        ...
        if not edited_df[['Adet', 'Maliyet']].equals(df_display[['Adet', 'Maliyet']]):
            ...
            for _, row in edited_df.iterrows():
                yeni_portfoy.append({"Hisse": row['Varlık'], "Adet": int(row['Adet']), "Maliyet": int(row['Maliyet'])})
            st.session_state.portfoy = yeni_portfoy
            save_data(yeni_portfoy)
            st.rerun()
    ...
    with tab_g:
        fig = px.pie(df, values='Değer', names='Varlık', title="Varlık Dağılımı", hole=.4)
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', font_color="white")
        st.plotly_chart(fig, use_container_width=True)

    with tab_t:
        st.markdown("### 💰 Temettü Beklentisi")
        tem_df = df[['Varlık', 'Adet', 'Temettü Verimi', 'Net Temettü']].copy()
        tem_df['Temettü Verimi (%)'] = tem_df['Temettü Verimi'].apply(lambda x: f"%{x:.2f}".replace('.', ','))
        tem_df['Yıllık Net Temettü (₺)'] = tem_df['Net Temettü'].apply(tr_format)
        
        st.dataframe(
            tem_df[['Varlık', 'Adet', 'Temettü Verimi (%)', 'Yıllık Net Temettü (₺)']],
    ...
    with tab_s:
        for _, r in df.iterrows():
            ...
            st.markdown(f'<div class="signal-box"><b>{r["Varlık"]}</b> | RSI: {r["RSI"]:.2f} | Tavsiye: {durum}</div>', unsafe_allow_html=True)
else:
    st.info("Portföy boş.")
...
st.caption(f"🕒 Son Güncelleme: {tr_saati} | Tam liste ve tüm özellikler aktif.")
