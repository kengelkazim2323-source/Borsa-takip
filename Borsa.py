...
# ==========================================
# 1. AYARLAR & PREMIUM TASARIM (YAN PANEL SİLİNDİ)
# ==========================================
st.set_page_config(page_title="Borsa Aslanı v10", page_icon="💎", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("""
    ...
    /* Üst Ticker Kartları */
    .top-ticker { background: linear-gradient(145deg, #161b22, #0d1117); border: 1px solid #30363d; border-radius: 10px; padding: 15px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
    .top-ticker h3 { margin: 0; font-size: 1rem; color: #8b949e !important; }
    .top-ticker p { margin: 5px 0 0 0; font-size: 1.4rem; font-weight: bold; color: #c9d1d9; }
    ...
    /* Gizli Sidebar (Sadece mobilde menü olarak kalır) */
...
# ==========================================
# 2. KOMPAKT ÜST PİYASA PANELİ
# ==========================================
st.markdown("<h1 style='text-align: center;'>🏛️ İMPARATOR YATIRIM TERMİNALİ</h1>", unsafe_allow_html=True)
...
for i, (isim, sembol) in enumerate(piyasa_hisseleri.items()):
    try:
        ...
        renk = "#3fb950" if degisim >= 0 else "#f85149"
        ...
        with cols[i]:
            ...
            <div class="top-ticker">
                <h3>{isim}</h3>
                <p>{fiyat:,.2f} <span style='font-size:0.6em; color:{renk};'>{ok} %{abs(degisim):.2f}</span></p>
            </div>
            """, unsafe_allow_html=True)
    except: continue
...
# ==========================================
# 3. ANA EKRANDA VARLIK GİRİŞİ (YATAY FORM)
# ==========================================
BIST_FULL = sorted([
...
# ==========================================
# 4. TEKNİK ANALİZ FONKSİYONU (RSI & SMA)
# ==========================================
