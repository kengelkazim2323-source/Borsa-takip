...
# ==========================================
# 0. SAYI FORMATLAMA (TR STANDARDI)
# ==========================================
def tr_format(val):
    """Sayıları 1.250,50 formatına çevirir."""
    try:
...
# ==========================================
# 2. TEMA VE SAAT (1 SANİYE GÜNCELLEME)
# ==========================================
...
# ==========================================
# 3. CANLI PİYASA (ONS GÜMÜŞ DAHİL)
# ==========================================
piyasa_izleme = {
    "GRAM ALTIN": "GAU-TRY", 
    "ONS ALTIN": "GC=F",
...
st.markdown(f"<h2 style='text-align: center; color:{main_color};'>🦁 BORSA ASLANI PORTFÖY</h2>", unsafe_allow_html=True)

with st.expander("➕ Yeni Hisse/Varlık Ekle", expanded=True):
    c1, c2, c3 = st.columns([2, 1, 1])
    with c1: s_varlik = st.selectbox("Hisse Seçin", BIST_FULL)
    with c2: s_adet = st.number_input("Adet", min_value=0.0, step=1.0)
    with c3: s_maliyet = st.number_input("Maliyet (TL)", min_value=0.0, format="%.2f")
...
if st.session_state.portfoy:
    ...
    for item in st.session_state.portfoy:
        try:
            ...
            if not div_rate and info.get('dividendYield'):
                div_rate = curr * info.get('dividendYield')
            ...
            p_list.append({
                ...
                "Değer": val, 
                "K/Z": kz,
                "Yıllık Temettü": div_rate * float(item['Adet'])
            })
        except: continue

    df = pd.DataFrame(p_list)
    tab1, tab2, tab3 = st.tabs(["📊 PORTFÖY TABLOSU", "📈 VARLIK DAĞILIMI", "💰 TEMETTÜ ANALİZİ"])

    with tab1:
        # Özet Kartlar
        m1, m2 = st.columns(2)
        total_val = df['Değer'].sum()
        total_kz = df['K/Z'].sum()
        m1.metric("TOPLAM PORTFÖY DEĞERİ", f"{tr_format(total_val)} ₺")
        m2.metric("TOPLAM KÂR / ZARAR", f"{tr_format(total_kz)} ₺", delta=f"{tr_format(total_kz)}")
        
        # Formatlanmış Tablo
        df_show = df.copy()
        for c in ["Maliyet", "Güncel", "Değer", "K/Z"]:
            df_show[c] = df_show[c].apply(tr_format)
        st.table(df_show[["Varlık", "Adet", "Maliyet", "Güncel", "Değer", "K/Z"]])

    with tab2:
        # Dairesel Grafik
        fig = px.pie(df, values='Değer', names='Varlık', hole=0.5, 
                     title="Portföy Dağılımı (TL Bazlı)",
                     color_discrete_sequence=px.colors.qualitative.Prism)
        fig.update_traces(textinfo='percent+label')
        st.plotly_chart(fig, use_container_width=True)

    with tab3:
        total_div = df['Yıllık Temettü'].sum()
        st.success(f"### Yıllık Beklenen Toplam Temettü: {tr_format(total_div)} ₺")
        st.info(f"Aylık Ortalama Ek Gelir: {tr_format(total_div/12)} ₺")
        
        t_list = df[df['Yıllık Temettü'] > 0][["Varlık", "Yıllık Temettü"]]
        if not t_list.empty:
            st.write("#### Hisse Bazlı Temettü Dağılımı")
            st.table(t_list.style.format({"Yıllık Temettü": "{:,.2f} ₺"}))
    ...
    st.warning("Henüz portföyünde varlık yok kral. Yukarıdan ekleme yapabilirsin.")
