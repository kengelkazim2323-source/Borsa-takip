...
# ==========================================
# 4. HİSSE EKLEME (TAM LİSTE İADE EDİLDİ)
# ==========================================
...
# ==========================================
# 5. LİSTELEME VE TEMETTÜ (MOBİL TABLO DÜZENLEMESİ)
# ==========================================
if st.session_state.portfoy:
    ...
    def portfoy_goster(piyasa_turu, tab_container, data_list):
        with tab_container:
            ...
            with st.expander("⚙️ VARLIKLARI YÖNET"):
                for _, r in df.iterrows():
    ...
    with tab_div:
        ...
        if not df_div.empty:
            ...
            div_html = '<div class="mobile-scroll"><table class="kral-table"><thead><tr><th>VARLIK</th><th>ADET</th><th>HİSSE BAŞI</th><th>NET GELİR</th></tr></thead><tbody>'
            for _, r in df_div.iterrows():
                if r['Temettu'] > 0:
                    birim = "₺" if r['Piyasa'] == "Türk Borsası" else "$"
                    div_html += f"<tr><td>{r['Hisse']}</td><td>{r['Adet']}</td><td>{tr_format(r['Temettu'])}</td><td><b>{tr_format(r['NetTemettu'])} {birim}</b></td></tr>"
    ...
    st.info("Portföy boş, ekleme yap.")
